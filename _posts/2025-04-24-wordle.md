...
<div class="wordle-intro">
  <div class="wordle-intro__header">
    <div class="wordle-logo" aria-hidden="true">
      <svg viewBox="0 0 200 64" role="presentation" focusable="false">
        <defs>
          <linearGradient id="wordleGradient" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stop-color="#c7f3ff"/>
            <stop offset="45%" stop-color="#9bd6ff"/>
            <stop offset="100%" stop-color="#7dc3ff"/>
          </linearGradient>
          <linearGradient id="wordleLeaf" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stop-color="#8bd48c"/>
            <stop offset="100%" stop-color="#4ea05e"/>
          </linearGradient>
        </defs>
        <rect x="0" y="8" width="200" height="44" rx="14" fill="url(#wordleGradient)" opacity="0.9"/>
        <circle cx="30" cy="30" r="20" fill="#0f1a2c" opacity="0.15"/>
        <path d="M35 28c4-6 14-10 22-8" stroke="#0f1a2c" stroke-width="3" stroke-linecap="round" opacity="0.15"/>
        <path d="M54 24c5.5 0 10-3.5 10-8s-6-6-9.5-2.5c-3-3.5-9.5-1.3-9.5 3.5s4.5 7 9 7z" fill="url(#wordleLeaf)" stroke="#1f5b33" stroke-width="1.4" stroke-linejoin="round"/>
        <text x="76" y="41" font-family="\"Segoe UI\", \"Helvetica Neue\", Arial" font-weight="800" font-size="30" fill="#0f172a" letter-spacing="1.5">WORDLE</text>
      </svg>
    </div>
    <div class="wordle-intro__eyebrow">Word puzzle</div>
    <h1>Wordle</h1>
    <p class="wordle-intro__lede">Guess the hidden five-letter word in six tries. Kid-friendly word list, on-screen keyboard, and a comfy full-screen play mode.</p>
    <div class="wordle-intro__actions">
      <button type="button" class="wordle-intro__cta" id="wordle-open">Play Wordle</button>
      <span class="wordle-intro__meta">Desktop + mobile friendly - Ghibli-inspired logo</span>
    </div>
  </div>
  ...
  <div class="wordle-intro__panels">
    <div class="wordle-intro__panel">
      <h3>How to play</h3>
      <ul>
        <li>Type a five-letter word and press Enter to submit.</li>
    ...
    <div class="wordle-intro__panel">
      ...
      <p class="wordle-preview__caption">Green = right letter, right spot. <br> Gold = right letter, wrong spot.</p>
    </div>
  </div>
  <p class="wordle-intro__note">Curated, kid-friendly word list (no scary or grown-up terms).</p>
</div>
...
.wordle-logo {
  max-width: 320px;
  margin: 0 0 0.4rem;
  filter: drop-shadow(0 10px 24px rgba(10, 22, 45, 0.28));
}
.wordle-logo svg {
  width: 100%;
  height: auto;
  display: block;
  animation: wordle-float 6s ease-in-out infinite;
}
.wordle-intro__lede {
  margin: 0;
  color: #cbdcf5;
...
<div class="wordle-overlay" id="wordle-overlay" aria-hidden="true">
  <div class="wordle-overlay__inner">
    ...
    <header class="wordle-play-header">
      <div>
        <p class="wordle-play-eyebrow">Play mode</p>
        <h2>Wordle</h2>
      </div>
      <div class="wordle-play-actions">
        <button id="wordle-reset" class="wordle-reset">New word</button>
      </div>
    </header>
...
.wordle-play-header h2 { margin: 0.2rem 0 0.25rem; font-weight: 800; }
.wordle-play-sub { margin: 0; color: #c7c9d4; }
.wordle-play-actions { display: flex; gap: 0.6rem; align-items: center; flex-wrap: wrap; }
.wordle-reset {
  border: none;
  background: #6aaa64;
  color: #0b1b0b;
  ...
  border-radius: 10px;
  cursor: pointer;
  box-shadow: 0 10px 25px rgba(106, 170, 100, 0.35);
}
.wordle-reset:hover { background: #7ac275; }
.wordle-reset:active { transform: translateY(1px); }
.wordle-stage {
  display: flex;
  flex-direction: column;
  align-items: center;
...
.wordle-keyboard-row:nth-child(2) { grid-template-columns: 0.7fr repeat(9, 1fr); }
...
@media (max-width: 720px) {
  .wordle-overlay__inner { padding-top: 48px; min-height: 100vh; border-radius: 0; }
  .wordle-play-header { align-items: flex-start; }
  .wordle-keyboard { width: 100%; position: sticky; bottom: 0; background: linear-gradient(180deg, transparent 0%, rgba(15,15,16,0.8) 40%, #0f0f10 100%); padding-bottom: 0.25rem; }
...
(function() {
  ...
  if (!boardEl || !keyboardEl || !messageEl || !resetBtn) {
    return;
  }
